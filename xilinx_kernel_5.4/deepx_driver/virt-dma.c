...
dma_cookie_t vchan_tx_submit(struct dma_async_tx_descriptor *tx)
{
	...
	dev_vdbg(vc->chan.device->dev, "vchan %p: txd %p[%x]: submitted\n",
		vc, vd, cookie);
...
int vchan_tx_desc_free(struct dma_async_tx_descriptor *tx)
{
	...
	dev_vdbg(vc->chan.device->dev, "vchan %p: txd %p[%x]: freeing\n",
		vc, vd, vd->tx.cookie);
	vc->desc_free(vd);
	return 0;
}
...
{
	struct virt_dma_chan *vc = container_of(t, struct virt_dma_chan, task);
	struct virt_dma_desc *vd, *_vd;
	struct dmaengine_desc_callback cb;
	LIST_HEAD(head);
	dev_vdbg(vc->chan.device->dev, "%s\n", __func__);
...
//	tasklet_init(&vc->task, vchan_complete);
