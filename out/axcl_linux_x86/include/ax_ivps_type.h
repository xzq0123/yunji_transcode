...
#define AX_IVPS_MIN_IMAGE_HEIGHT 8
...
#define AX_IVPS_MIN_POLYGON_POINT_NUM  4

#define AX_IVPS_USER_FRAME_RATE_NUM  70
...
typedef struct
{
    ...
    AX_BOOL bHwRotation; /* Only for DSP hardware Rotation*/
    AX_VDSP_MODE_E eVdspMode; /* Only for DSP's core select */
    AX_BOOL bMirror;
    AX_BOOL bFlip;
    union
    {
...
typedef struct
{
    AX_BOOL bEnable;
    AX_U8   nArryNum;
    AX_BOOL bRateCtrl[AX_IVPS_USER_FRAME_RATE_NUM];
} AX_IVPS_USER_FRAME_RATE_CTRL_T;
...
/* acc:10bit, scale_step = input / output * 1024 */
typedef struct {
    AX_BOOL bEnable;
    AX_U16 nScaleStepW;
    AX_U16 nScaleStepH;
} AX_IVPS_SCALE_STEP_T;
