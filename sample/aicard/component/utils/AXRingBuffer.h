...
public:
    ...
    CAXRingElement(AX_U8* pBuf, AX_U32 nSize, AX_U64 u64PTS = 0, AX_BOOL isIFrame = AX_FALSE, AX_U8* pHeadBuf = nullptr,
                   AX_U32 nHeadSize = 0) {
        this->nIndex = -1;
        this->pBuf = pBuf;
        this->nSize = nSize;
    ...
    AX_VOID CopyFrom(CAXRingElement& element) {
        ...
        this->nSize = element.nSize + element.nHeadSize;
...
public:
    ...
    AX_BOOL Put(CAXRingElement& element) {
        ...
        if (!m_pRing || (element.nSize + element.nHeadSize) > m_nElementBuffSize) {
            ...
            LOG_MM_E(AXRING, "[%s] ---, drop one frame for size: %d is bigger than buffer size(%d)", m_szName, element.nSize,
                  m_nElementBuffSize);
            return AX_FALSE;
        }
        ...
        if ((m_nTail - m_nHeader) == m_nElementCount) {
            // is full
            LOG_MM_I(AXRING, "[%s] is full, tail=%llu, head=%llu", m_szName, m_nTail, m_nHeader);
            if (element.bIFrame) {
                ...
                if (nRefCount == 0) {
                    m_nTail--;
                    m_bHasLost = AX_FALSE;
                    LOG_MM_I(AXRING, "[%s] ---, replace last one with i frame for is full", m_szName);
                } else {
                    m_bHasLost = AX_TRUE;
                    LOG_MM_I(AXRING, "[%s] ---, drop one i frame for is full", m_szName);
                    return AX_FALSE;
                }

            } else {
                m_bHasLost = AX_TRUE;  // mark to lost all behind P Frame
                LOG_MM_I(AXRING, "[%s] ---, drop one p frame for is full", m_szName);
                return AX_FALSE;
            }
            ...
            if (element.bIFrame) {
                ...
                if (m_bHasLost) {
                    // drop this P Frame
                    LOG_MM_I(AXRING, "[%s] ---, drop one p frame for lost yet", m_szName);
                    return AX_FALSE;
                }
            }
        }
        ...
        if (nRefCount != 0) {
            m_bHasLost = AX_TRUE;
            LOG_MM_I(AXRING, "[%s] ---, drop one frame for refcount is none zero, nIndex=%llu, nRefCount=%d", m_szName, nIndex, nRefCount);
            return AX_FALSE;
        }
        ...
        // LOG_M(AXRING, "[%s] ---, tail=%llu, head=%llu, pBuf=%p, size=%u", m_szName, m_nTail, m_nHeader, m_pRing[nIndex].pBuf,
        // m_pRing[nIndex].nSize);
        return AX_TRUE;
    }
    ...
    CAXRingElement* Get() {
        ...
        // LOG_M(AXRING, "[%s] ---, tail=%llu, head=%llu, pBuf=%p, size=%u, nRef=%d", m_szName, m_nTail, m_nHeader, element->pBuf,
        // element->nSize, nRefCount);
        return element;
    }
