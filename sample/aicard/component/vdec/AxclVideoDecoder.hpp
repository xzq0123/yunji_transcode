...
typedef struct AXCL_VDEC_GRP_ATTR_S {
    AX_BOOL bEnable;
...
} AXCL_VDEC_GRP_ATTR_T;

/**
 * @brief
 *
 */
class CAxclStreamCacheBuf;
class CAxclVideoDecoder : public IStreamObserver {
public:
    CAxclVideoDecoder(AX_VOID) noexcept = default;
    ...
    AX_BOOL Init(AX_S32 nDevID, const std::vector<AXCL_VDEC_GRP_ATTR_T> &v);
    ...
    AX_BOOL GetGrpAttr(AX_VDEC_GRP vdGrp, AXCL_VDEC_GRP_ATTR_T &stGrpAttr) const;
    AX_BOOL GetAllGrpAttr(std::vector<AXCL_VDEC_GRP_ATTR_T> &vecGrpAttr);
...
protected:
    ...
    std::vector<std::unique_ptr<CAxclStreamCacheBuf>> m_arrCacheBuf;
    std::mutex m_mtxStop;
};

class CAxclStreamCacheBuf {
public:
    explicit CAxclStreamCacheBuf(AX_U32 nCapacity) noexcept : m_nCapacity(nCapacity) {
        if (0 == m_nCapacity) {
