...
AX_VOID CAxVideoDecoder::RecvThread(AX_VOID* pArg) {
...
AX_U32 CAxVideoDecoder::GetBlkSize(AX_U32 nW, AX_U32 nH, AX_U32 nAlign, AX_PAYLOAD_TYPE_E eType,
...
AX_BOOL CAxVideoDecoder::Init(const std::vector<VDEC_GRP_ATTR_T>& v) {
    LOG_M_D(VDEC, "%s: +++", __func__);
...
AX_BOOL CAxVideoDecoder::DeInit(AX_VOID) {
    LOG_M_D(VDEC, "%s: +++", __func__);
...
AX_BOOL CAxVideoDecoder::GetGrpAttr(AX_VDEC_GRP vdGrp, VDEC_GRP_ATTR_T& stGrpAttr) const {
...
AX_BOOL CAxVideoDecoder::GetAllGrpAttr(std::vector<VDEC_GRP_ATTR_T>& vecGrpAttr) {
...
AX_BOOL CAxVideoDecoder::Start(AX_VOID) {
    LOG_M_D(VDEC, "%s: +++", __func__);
...
AX_BOOL CAxVideoDecoder::Stop(AX_VOID) {
    LOG_M_D(VDEC, "%s: +++", __func__);
...
AX_BOOL CAxVideoDecoder::RegObserver(AX_VDEC_GRP vdGrp, IObserver* pObs) {
    if (!pObs) {
...
AX_BOOL CAxVideoDecoder::UnRegObserver(AX_VDEC_GRP vdGrp, IObserver* pObs) {
    if (!pObs) {
        LOG_M_E(VDEC, "%s observer is nil", __func__);
        return AX_FALSE;
    }
...
AX_BOOL CAxVideoDecoder::UnRegAllObservers(AX_VOID) {
    std::lock_guard<std::mutex> lck(m_mtxObs);
    m_mapObs.clear();
    return AX_TRUE;
}

AX_BOOL CAxVideoDecoder::CreateDecoder(AX_VDEC_GRP vdGrp, const VDEC_GRP_INFO_T& stGrpInfo) {
...
AX_BOOL CAxVideoDecoder::OnRecvVideoData(AX_S32 nCookie, const AX_U8* pData, AX_U32 nLen, AX_U64 nPTS) {
...
AX_BOOL CAxVideoDecoder::OnRecvAudioData(AX_S32 nCookie, const AX_U8* pData, AX_U32 nLen, AX_U64 nPTS) {
    return AX_TRUE;
}

AX_BOOL CAxVideoDecoder::Send(AX_VDEC_GRP vdGrp, const AX_U8* pData, AX_U32 nLen, AX_U64 nPTS) {
...
AX_BOOL CAxVideoDecoder::Notify(const CAXFrame& axFrame) {
    std::lock_guard<std::mutex> lck(m_mtxObs);
...
AX_BOOL CAxVideoDecoder::AttachPool(AX_VDEC_GRP vdGrp, AX_VDEC_CHN vdChn, AX_POOL pool) {
...
AX_BOOL CAxVideoDecoder::DetachPool(AX_VDEC_GRP vdGrp, AX_VDEC_CHN vdChn) {
...
AX_BOOL CAxVideoDecoder::SetChnAttr(AX_VDEC_GRP vdGrp, AX_VDEC_CHN vdChn, const AX_VDEC_CHN_ATTR_T& stChnAttr) {
...
AX_BOOL CAxVideoDecoder::ResetGrp(AX_VDEC_GRP vdGrp, AX_S32 nTryCount /* = AICARD_VDEC_RESET_GRP_TRY_COUNT*/) {
