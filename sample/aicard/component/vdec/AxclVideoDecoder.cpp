...
AX_VOID CAxclVideoDecoder::RecvThread(AX_VOID* pArg) {
    ...
    while (m_DecodeThread.IsRunning()) {
        ...
        ret = AXCL_VDEC_SelectGrp(&stGrpSet, 1000);
        if (0 != ret) {
            if (AX_ERR_VDEC_FLOW_END == ret) {
                LOG_M_W(VDEC, "AXCL_VDEC_SelectGrp() recv flow end");
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
            } else {
                if (AX_ERR_VDEC_TIMED_OUT != ret) {
                    LOG_M_E(VDEC, "AXCL_VDEC_SelectGrp() fail, ret = 0x%x", ret);
                } else {
                    LOG_M_I(VDEC, "AXCL_VDEC_SelectGrp() timeout");
                }
            }
            continue;
        }
        ...
        if (0 == stGrpSet.u32GrpCount) {
            ...
            LOG_M_E(VDEC, "AXCL_VDEC_SelectGrp() success, but return vdGrp count is 0");
            break;
        }
        ...
        for (AX_U32 i = 0; i < stGrpSet.u32GrpCount; ++i) {
            for (AX_U32 j = 0; j < stGrpSet.stChnSet[i].u32ChnCount; ++j) {
                ...
                if (vdGrp < 0 || vdChn < 0) {
                    /* how could this happen, just make sure */
                    LOG_M_E(VDEC, "AXCL_VDEC_SelectGrp() success, but return invalid vdGrp %d or vdChn %d", vdGrp, vdChn);
                    goto _EXIT_PROC;
                }
                ...
                if (1 == ++tChnInfo.nSeqNum || 0 == tChnInfo.nFps) {
                    ...
                    AXCL_SYS_GetCurPTS(&tChnInfo.nPTS);
                } else {
                    tChnInfo.nPTS += (PTS_TIME_BASE / tChnInfo.nFps);
                }

                AX_VIDEO_FRAME_INFO_T stVFrame;
                ret = AXCL_VDEC_GetChnFrame(vdGrp, vdChn, &stVFrame, TIMEOUT);
                if (0 != ret) {
                    if (AX_ERR_VDEC_FLOW_END == ret || AX_ERR_VDEC_UNEXIST == ret) {
                        ...
                        LOG_M_N(VDEC, "AXCL_VDEC_GetChnFrame(vdGrp %d, vdChn %d): stream is undecodeable", vdGrp, vdChn);
                        continue;
                    } else {
                        LOG_M_E(VDEC, "AXCL_VDEC_GetChnFrame(vdGrp %d, vdChn %d, timeout %d) fail, ret = 0x%x", vdGrp, vdChn, TIMEOUT, ret);
                        goto _EXIT_PROC;
                    }
                }
                ...
                if (AX_INVALID_BLOCKID == stVFrame.stVFrame.u32BlkId[0]) {
                    LOG_M_E(VDEC, "AXCL_VDEC_GetChnFrame(vdGrp %d, vdChn %d) recv invalid frame blk id", vdGrp, vdChn);
                    goto _EXIT_PROC;
                }
                ...
                if (0 == stVFrame.stVFrame.u32Width || 0 == stVFrame.stVFrame.u32Height ||
                    ...
                    LOG_M_W(VDEC, "AXCL_VDEC_GetChnFrame(vdGrp %d, vdChn %d) recv invalid frame %dx%d, pxl fmt %d, blk 0x%x", vdGrp, vdChn,
                    ...
                    ret = AXCL_VDEC_ReleaseChnFrame(vdGrp, vdChn, &stVFrame);
                    if (0 != ret) {
                        LOG_M_E(VDEC, "AXCL_VDEC_ReleaseChnFrame(vdGrp %d, vdChn %d) fail, ret = 0x%x", vdGrp, vdChn, ret);
                        goto _EXIT_PROC;
                    }
...
AX_U32 CAxclVideoDecoder::GetBlkSize(AX_U32 nW, AX_U32 nH, AX_U32 nAlign, AX_PAYLOAD_TYPE_E eType,
...
AX_BOOL CAxclVideoDecoder::Init(AX_S32 nDevID, const std::vector<AXCL_VDEC_GRP_ATTR_T>& v) {
    LOG_M_D(VDEC, "%s: +++", __func__);
    m_nDevID = nDevID;

    if (0 == count_if(v.begin(), v.end(), [](const AXCL_VDEC_GRP_ATTR_T& m) { return m.bEnable ? true : false; })) {
    ...
    for (AX_VDEC_GRP vdGrp = 0; vdGrp < (AX_VDEC_GRP)VDEC_GRP_NUM; ++vdGrp) {
        ...
        if (AX_VDEC_INPUT_MODE_STREAM == stGrpInfo.stAttr.enInputMode) {
            m_arrCacheBuf[vdGrp] = make_unique<CAxclStreamCacheBuf>(stGrpInfo.stAttr.nMaxStreamBufSize);
...
AX_BOOL CAxclVideoDecoder::DeInit(AX_VOID) {
    LOG_M_C(VDEC, "%s: +++", __func__);
    ...
    for (AX_VDEC_GRP vdGrp = 0; vdGrp < (AX_VDEC_GRP)VDEC_GRP_NUM; ++vdGrp) {
        ...
        if (m_arrGrpInfo[vdGrp].bActive) {
            // LOG_M_W(VDEC, "AXCL_VDEC_DestroyGrp(vdGrp %d) +++", vdGrp);
            // for (AX_U32 i = 0; i < 3; ++i) {
            //     ret = AXCL_VDEC_DestroyGrp(vdGrp);
            //     if (0 == ret) {
            //         break;
            //     } else {
            //         LOG_M_W(VDEC, "AXCL_VDEC_DestroyGrp(vdGrp %d) fail %d times", vdGrp, i + 1);
            //         std::this_thread::sleep_for(std::chrono::milliseconds(100));
            //     }
            // }
            // // LOG_M_W(VDEC, "AXCL_VDEC_DestroyGrp(vdGrp %d) ---, ret = 0x%x", vdGrp, ret);
            // if (0 != ret) {
            //     LOG_M_E(VDEC, "AXCL_VDEC_DestroyGrp(vdGrp %d) fail, ret = 0x%x", vdGrp, ret);
            // }
...
AX_BOOL CAxclVideoDecoder::GetGrpAttr(AX_VDEC_GRP vdGrp, AXCL_VDEC_GRP_ATTR_T& stGrpAttr) const {
...
AX_BOOL CAxclVideoDecoder::GetAllGrpAttr(std::vector<AXCL_VDEC_GRP_ATTR_T>& vecGrpAttr) {
...
AX_BOOL CAxclVideoDecoder::Start(AX_VOID) {
    LOG_M_D(VDEC, "%s: +++", __func__);
    ...
    for (AX_VDEC_GRP vdGrp = 0; vdGrp < (AX_VDEC_GRP)VDEC_GRP_NUM; ++vdGrp) {
        ...
        ret = AXCL_VDEC_StartRecvStream(vdGrp, &stRecvParam);
        if (0 != ret) {
            LOG_M_E(VDEC, "AXCL_VDEC_StartRecvStream(vdGrp %d) fail, ret = 0x%x", vdGrp);
            Stop();
            return AX_FALSE;
        }
...
AX_BOOL CAxclVideoDecoder::Stop(AX_VOID) {
    LOG_M_C(VDEC, "%s: +++", __func__);
    ...
    for (AX_VDEC_GRP vdGrp = 0; vdGrp < (AX_VDEC_GRP)VDEC_GRP_NUM; ++vdGrp) {
        ...
        ret = AXCL_VDEC_SendStream(vdGrp, &tStrInfo, -1);
        if (ret != AX_SUCCESS) {
            LOG_M_E(VDEC, "vdGrp %d send eof frame failed, ret:0x%x", vdGrp, ret);
        }
        else {
            LOG_M_C(VDEC, "vdGrp %d send eof frame succ", vdGrp);
        }
    }

    {
        ...
        for (AX_VDEC_GRP vdGrp = 0; vdGrp < (AX_VDEC_GRP)VDEC_GRP_NUM; ++vdGrp) {
            ...
            ret = AXCL_VDEC_QueryStatus(vdGrp, &vdGrpStatus);
            if (0 != ret) {
                LOG_M_E(VDEC, "%s: AXCL_VDEC_QueryStatus(vdGrp %d) fail, ret = 0x%x", __func__, vdGrp, ret);
            } else {
                if (vdGrpStatus.bStartRecvStream) {
                    LOG_M_C(VDEC, "stop vdGrp %d +++", vdGrp);

                    ret = AXCL_VDEC_StopRecvStream(vdGrp);
                    if (0 != ret) {
                        LOG_M_E(VDEC, "%s: AXCL_VDEC_StopRecvStream(vdGrp %d) fail, ret = 0x%x", __func__, vdGrp, ret);
                        return AX_FALSE;
                    }
                    ...
                    ret = AXCL_VDEC_DestroyGrp(vdGrp);
                    if (0 != ret) {
                        LOG_M_E(VDEC, "%s: AXCL_VDEC_DestroyGrp(vdGrp %d) fail, ret = 0x%x", __func__, vdGrp, ret);
                    }
#if 0
                    AX_VDEC_RECV_PIC_PARAM_T stRecvParam;
                    memset(&stRecvParam, 0, sizeof(stRecvParam));
                    ...
                    ret = AXCL_VDEC_StartRecvStream(vdGrp, &stRecvParam);
                    if (0 != ret) {
                        LOG_M_E(VDEC, "%s: AXCL_VDEC_StartRecvStream(vdGrp %d) fail, ret = 0x%x", __func__, vdGrp, ret);
                        return AX_FALSE;
                    }
                    ...
                    ret = AXCL_VDEC_SendStream(vdGrp, &eof, -1);
                    if (0 != ret) {
                        LOG_M_E(VDEC, "%s: AXCL_VDEC_SendStream(vdGrp %d eof) fail, ret = 0x%x", __func__, vdGrp, ret);
                        return AX_FALSE;
                    }
                    ...
                    LOG_M_C(VDEC, "vdGrp %d not in recv status, do nothing", vdGrp);
                }
            }
        }
...
AX_BOOL CAxclVideoDecoder::RegObserver(AX_VDEC_GRP vdGrp, IObserver* pObs) {
    if (!pObs) {
...
AX_BOOL CAxclVideoDecoder::UnRegObserver(AX_VDEC_GRP vdGrp, IObserver* pObs) {
    if (!pObs) {
        LOG_M_E(VDEC, "%s observer is nil", __func__);
        return AX_FALSE;
    }
...
AX_BOOL CAxclVideoDecoder::UnRegAllObservers(AX_VOID) {
    std::lock_guard<std::mutex> lck(m_mtxObs);
    m_mapObs.clear();
    return AX_TRUE;
}

AX_BOOL CAxclVideoDecoder::CreateDecoder(AX_VDEC_GRP vdGrp, const VDEC_GRP_INFO_T& stGrpInfo) {
    ...
    AX_S32 ret = AXCL_VDEC_CreateGrp(vdGrp, &stGrpAttr);
    if (0 != ret) {
        LOG_M_E(VDEC, "AXCL_VDEC_CreateGrp(vdGrp %d) fail, ret = 0x%x", vdGrp, ret);
        return AX_FALSE;
    }
        ...
        */
...
#endif
        ret = AXCL_VDEC_SetGrpParam(vdGrp, &stGrpParam);
        if (0 != ret) {
            LOG_M_E(VDEC, "AXCL_VDEC_SetGrpParam(vdGrp %d) fail, ret = 0x%x", vdGrp, ret);
            throw 1;
        }

        ret = AXCL_VDEC_SetDisplayMode(vdGrp, stGrpInfo.stAttr.eDecodeMode);
        if (0 != ret) {
            LOG_M_E(VDEC, "AXCL_VDEC_SetDisplayMode(vdGrp %d) fail, ret = 0x%x", vdGrp, ret);
            throw 1;
        }
        ...
        for (AX_VDEC_CHN vdChn = 0; vdChn < MAX_VDEC_CHN_NUM; ++vdChn) {
            if (stGrpInfo.stAttr.bChnEnable[vdChn]) {
                ...
                ret = AXCL_VDEC_SetChnAttr(vdGrp, vdChn, &stChnAttr);
                if (0 != ret) {
                    LOG_M_E(VDEC, "AXCL_VDEC_SetChnAttr(vdGrp %d, vdChn %d) fail, ret = 0x%x", vdGrp, vdChn, ret);
                    throw 1;
                }

                ret = AXCL_VDEC_EnableChn(vdGrp, vdChn);
                if (0 != ret) {
                    LOG_M_E(VDEC, "AXCL_VDEC_EnableChn(vdGrp %d, vdChn %d) fail, ret = 0x%x", vdGrp, vdChn, ret);
                    throw 1;
                }
            } else {
                ret = AXCL_VDEC_DisableChn(vdGrp, vdChn);
                if (0 != ret) {
                    LOG_M_E(VDEC, "AXCL_VDEC_DisableChn(vdGrp %d, vdChn %d) fail, ret = 0x%x", vdGrp, vdChn, ret);
                    throw 1;
                }
            }
        }
...
AX_BOOL CAxclVideoDecoder::OnRecvVideoData(AX_S32 nCookie, const AX_U8* pData, AX_U32 nLen, AX_U64 nPTS) {
...
AX_BOOL CAxclVideoDecoder::OnRecvAudioData(AX_S32 nCookie, const AX_U8* pData, AX_U32 nLen, AX_U64 nPTS) {
    return AX_TRUE;
}

AX_BOOL CAxclVideoDecoder::Send(AX_VDEC_GRP vdGrp, const AX_U8* pData, AX_U32 nLen, AX_U64 nPTS) {
    ...
    // LOG_M_C(VDEC, "AXCL_VDEC_SendStream(vdGrp %d len %d) +++", vdGrp, stStream.u32StreamPackLen);
    AX_S32 ret = AXCL_VDEC_SendStream(vdGrp, &stStream, -1);
    ...
    if (0 != ret) {
        if (AX_ERR_VDEC_NEED_REALLOC_BUF == ret) {
            ...
            ret = AXCL_VDEC_StopRecvStream(vdGrp);
            if (0 != ret) {
                LOG_M_E(VDEC, "AXCL_VDEC_StopRecvStream(vdGrp %d) fail, ret = 0x%x", vdGrp, ret);
                return AX_FALSE;
            }
            ...
            LOG_M_E(VDEC, "AXCL_VDEC_SendStream(vdGrp %d) fail, ret = 0x%x", vdGrp, ret);
        }
...
AX_BOOL CAxclVideoDecoder::Notify(const CAXFrame& axFrame) {
    std::lock_guard<std::mutex> lck(m_mtxObs);
...
AX_BOOL CAxclVideoDecoder::AttachPool(AX_VDEC_GRP vdGrp, AX_VDEC_CHN vdChn, AX_POOL pool) {
    if (AX_INVALID_POOLID == pool) {
        LOG_M_E(VDEC, "AttachPool: invalid pool id %d", pool);
        return AX_FALSE;
    }

    AX_S32 ret = AXCL_VDEC_AttachPool(vdGrp, vdChn, pool);
    if (0 != ret) {
        LOG_M_E(VDEC, "AXCL_VDEC_AttachPool(vdGrp %d, vdChn %d) pool %d fail, ret = 0x%x", vdGrp, vdChn, pool, ret);
        return AX_FALSE;
    }
...
AX_BOOL CAxclVideoDecoder::DetachPool(AX_VDEC_GRP vdGrp, AX_VDEC_CHN vdChn) {
    AX_S32 ret = AXCL_VDEC_DetachPool(vdGrp, vdChn);
    if (0 != ret) {
        LOG_M_E(VDEC, "AXCL_VDEC_DetachPool(vdGrp %d, vdChn %d) fail, ret = 0x%x", vdGrp, vdChn, ret);
        return AX_FALSE;
    }
...
AX_BOOL CAxclVideoDecoder::SetChnAttr(AX_VDEC_GRP vdGrp, AX_VDEC_CHN vdChn, const AX_VDEC_CHN_ATTR_T& stChnAttr) {
    AX_S32 ret = AXCL_VDEC_SetChnAttr(vdGrp, vdChn, &stChnAttr);
    if (0 != ret) {
        LOG_M_E(VDEC, "AXCL_VDEC_SetChnAttr(vdGrp %d, vdChn %d) fail, ret = 0x%x", vdGrp, vdChn, ret);
        return AX_FALSE;
    }
...
AX_BOOL CAxclVideoDecoder::ResetGrp(AX_VDEC_GRP vdGrp, AX_S32 nTryCount /* = AICARD_VDEC_RESET_GRP_TRY_COUNT*/) {
    AX_S32 ret = AX_SUCCESS;
    AX_S32 nIndex = 0;
    while (nIndex++ < nTryCount) {
        ret = AXCL_VDEC_ResetGrp(vdGrp);
        if (0 != ret) {
            if (AX_ERR_VDEC_BUSY == ret) {
                LOG_M_W(VDEC, "[%d] vdGrp %d is busy, try again to reset", nIndex, vdGrp);
            } else {
                LOG_M_E(VDEC, "[%d] Try AXCL_VDEC_ResetGrp(vdGrp %d) fail, ret = 0x%x", nIndex, vdGrp, ret);
            }
    ...
    if (AX_SUCCESS != ret) {
        LOG_M_E(VDEC, "AXCL_VDEC_ResetGrp(vdGrp %d) failed after try %d times, ret = 0x%x", vdGrp, nTryCount, ret);
        return AX_FALSE;
    }
