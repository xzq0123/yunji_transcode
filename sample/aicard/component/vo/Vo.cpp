...
AX_BOOL CVo::Init(const VO_ATTR_T& stAttr) {
    ...
    if (m_arrChns.size() > 0) {
        m_rectWidth = m_arrChns[0].stRect.u32Width;
        m_rectHeight = m_arrChns[0].stRect.u32Height;

        AX_U32 u32BlkCnt = 8;
        AX_U64 u64BlkSize = ALIGN_UP(m_rectWidth, 16) * m_rectHeight * 3 / 2;
        AX_U64 u64MetaSize = 512;

        if (!CreatePrivatePool(u32BlkCnt, u64BlkSize, u64MetaSize)) {
            return AX_FALSE;
        }
    }
...
AX_BOOL CVo::CreatePrivatePool(AX_U32 u32BlkCnt, AX_U64 u64BlkSize, AX_U64 u64MetaSize) {
    for (AX_U32 i = 0;i < m_nVideoChnCount;i++) {
        ...
        m_priPoolID[i] = AX_POOL_CreatePool(&stPoolCfg);
        if (m_priPoolID[i] == AX_INVALID_POOLID) {
            LOG_MM_E(VO, "AX_POOL_CreatePool failed, u32BlkCnt = %d, u64BlkSize = 0x%llx, u64MetaSize = 0x%llx\n", u32BlkCnt,
                     u64BlkSize, u64MetaSize);
            return AX_FALSE;
        }

        // LOG_MM_C(VO, "u32BlkCnt = %d, u64BlkSize = 0x%llx, pPoolID = %d\n", u32BlkCnt, u64BlkSize, m_priPoolID[i]);
    }
    return AX_TRUE;
}
