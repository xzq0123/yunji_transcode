...
AX_POOL IVPS_UserPoolCreate(AX_U32 nBlkSize, AX_U32 nBlkCnt)
{
    ...
    PoolId = AXCL_POOL_CreatePool(&PoolConfig);
    ...
    ALOGI("success AXCL Create PoolId:%d", PoolId);
    return PoolId;
}
...
AX_S32 BufDestroy(AX_POOL PoolId)
{
    ...
    ret = AXCL_POOL_DestroyPool(PoolId);
    if (ret)
    {
        ALOGE("fail!!Error Code:0x%X", ret);
        return -1;
    }
    ALOGI("success AXCL pool id:%d!", PoolId);
...
AX_S32 BufDestroyAll(AX_VOID)
{
    ...
    for (i = BufPoolCnt - 1; i >= 0; i--)
    {
        ...
        ret = AXCL_POOL_DestroyPool(tBufPool[i].PoolId);
        if (ret)
        {
            ALOGE("fail!!Error Code:0x%X", ret);
            return -1;
...
AX_S32 BufPoolBlockAddrGet(AX_POOL PoolId, AX_U32 BlkSize, AX_U64 *nPhyAddr, AX_VOID **pVirAddr, AX_BLK *BlkId)
{
    ...
    *BlkId = AXCL_POOL_GetBlock(PoolId, BlkSize, (const AX_S8*)"anonymous");

    *nPhyAddr = AXCL_POOL_Handle2PhysAddr(*BlkId);
    ...
    *pVirAddr = AXCL_POOL_GetBlockVirAddr(*BlkId);
...
AX_BOOL LoadImage(const char *pszImge, AX_U32 pImgSize, AX_U64 *pPhyAddr, AX_VOID **ppVirAddr)
{
    ...
    if (fp)
    {
        ...
        ret = AXCL_SYS_MemAlloc((AX_U64 *)pPhyAddr, ppVirAddr, nFileSize, SAMPLE_PHY_MEM_ALIGN_SIZE, NULL);
        if (0 != ret)
        {
        ...
        axclrtMallocHost(&HostMem, nFileSize);
        size_t sz = fread(HostMem, 1, nFileSize, fp);
        fclose(fp);
        if (sz != (size_t)nFileSize) {
            axclrtFreeHost(HostMem);
            ALOGE("read %ld size from file, target size is %d", sz, nFileSize);
            return AX_FALSE;
        }

        ret = axclrtMemcpy((void *)*pPhyAddr, HostMem, nFileSize, AXCL_MEMCPY_HOST_TO_DEVICE);
        if (ret)
        {
                            ...
                            AX_BLK *BlkId, AX_BOOL isYUVSplit, AX_U64 *nPhyAddr, AX_VOID **pVirAddr)
{
    ...
    axclrtMallocHost(&HostMem, nImgSize);
    size_t sz = fread(HostMem, 1, nImgSize, fp);
    fclose(fp);
    if (sz != (size_t)nImgSize) {
        axclrtFreeHost(HostMem);
        ALOGE("read %ld size from image, target size is %d", sz, nImgSize);
        return AX_FALSE;
    }

    ret = axclrtMemcpy((void *)*nPhyAddr, HostMem, nImgSize, AXCL_MEMCPY_HOST_TO_DEVICE);
    if (ret)
    {
        ALOGE("IVPS Release BlkId fail, ret=0x%x", ret);
        return AX_FALSE;
                 ...
                 char *pFilePath, char *pFileName, char *pCaseId)
{
    ...
    case 2:
        ...
        if (AX_FORMAT_YUV422_SEMIPLANAR == tDstFrame->enImgFormat
            ...
            axclrtMallocHost(&host_mem[0], nPixelSize);
            axclrtMallocHost(&host_mem[1], nPixelSize);
            axclrtMemcpy(host_mem[0], (void *)tDstFrame->u64PhyAddr[0], nPixelSize, AXCL_MEMCPY_DEVICE_TO_HOST);
            axclrtMemcpy(host_mem[1], (void *)tDstFrame->u64PhyAddr[1], nPixelSize, AXCL_MEMCPY_DEVICE_TO_HOST);
            fwrite((AX_VOID *)((AX_ULONG)host_mem[0]), 1, nPixelSize, fp);
            fwrite((AX_VOID *)((AX_ULONG)host_mem[1]), 1, nPixelSize, fp);
        } else {
            axclrtMallocHost(&host_mem[0], nPixelSize);
            axclrtMallocHost(&host_mem[1], nPixelSize / 2);
            axclrtMemcpy(host_mem[0], (void *)tDstFrame->u64PhyAddr[0], nPixelSize, AXCL_MEMCPY_DEVICE_TO_HOST);
            axclrtMemcpy(host_mem[1], (void *)tDstFrame->u64PhyAddr[1], nPixelSize / 2, AXCL_MEMCPY_DEVICE_TO_HOST);
            fwrite((AX_VOID *)((AX_ULONG)host_mem[0]), 1, nPixelSize, fp);
            fwrite((AX_VOID *)((AX_ULONG)host_mem[1]), 1, nPixelSize / 2, fp);
        }
        break;
    // case 3:
    //     tDstFrame->u64VirAddr[0] = (AX_ULONG)AXCL_SYS_Mmap(tDstFrame->u64PhyAddr[0], nPixelSize);
    //     tDstFrame->u64VirAddr[1] = (AX_ULONG)AXCL_SYS_Mmap(tDstFrame->u64PhyAddr[1], nPixelSize / 2);
    //     tDstFrame->u64VirAddr[2] = (AX_ULONG)AXCL_SYS_Mmap(tDstFrame->u64PhyAddr[2], nPixelSize / 2);
    ...
    //     s32Ret1 = AXCL_SYS_Munmap((AX_VOID *)(AX_ULONG)tDstFrame->u64VirAddr[0], nPixelSize);
    //     s32Ret2 = AXCL_SYS_Munmap((AX_VOID *)(AX_ULONG)tDstFrame->u64VirAddr[1], nPixelSize / 2);
    //     s32Ret3 = AXCL_SYS_Munmap((AX_VOID *)(AX_ULONG)tDstFrame->u64VirAddr[2], nPixelSize / 2);
    //     break;
    default:
        if (tDstFrame->u32FrameSize)
        {
            axclrtMallocHost(&host_mem[0], tDstFrame->u32FrameSize);
            axclrtMemcpy(host_mem[0], (void *)tDstFrame->u64PhyAddr[0], tDstFrame->u32FrameSize, AXCL_MEMCPY_DEVICE_TO_HOST);
            fwrite((AX_VOID *)((AX_ULONG)host_mem[0]), 1, tDstFrame->u32FrameSize, fp);
        }
        else
        {
            /* eg: rgb888 */
