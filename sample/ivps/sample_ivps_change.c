	...
	/* Only change filter1, filter0 don't change. */
        for (filter_idx = 1; filter_idx < 2; filter_idx++) {
            if (pGrp->tPipelineAttr.tFilter[pipe_idx][filter_idx].bEngage) {
                ...
                AXCL_IVPS_GetChnAttr(pGrp->nIvpsGrp, pipe_idx - 1, filter_idx, &ChnAttr);
                ...
                AXCL_IVPS_SetChnAttr(pGrp->nIvpsGrp, pipe_idx - 1, filter_idx, &ChnAttr);
	    }
...
static AX_VOID *IVPS_FilterAttrChangeThread(AX_VOID *pArg)
{
    ...
    sleep(1);
    AXCL_IVPS_GetPipelineAttr(pGrp->nIvpsGrp, &pGrp->tPipelineAttr);
