...
static AX_CHAR optstr[] = "?::c:e:m:t:i:o:w:h:p:d:a:";
...
AX_VOID SAMPLE_IVE_Usage(AX_CHAR* pchPrgName)
{
    ...
    printf("\t-d | --slave device ID, optional \n");
...
int main(int argc, char *argv[])
{
    ...
    if (devID <= 0) {
        ...
        devID = lst.devices[0];
        SAMPLE_IVE_PRT("device id: %d", devID);
    }

    s32Ret = axclrtSetDevice(devID);
    if (AXCL_SUCC != s32Ret) {
        axclFinalize();
        SAMPLE_IVE_PRT("Fail to set Device, device id: %d", devID);
        return s32Ret;
    }
    ...
    case 17: { //MatMul
            ...
            for (int i = 0; i < AX_ENGINE_VIRTUAL_NPU_BUTT; i++) {
                ...
                AX_S32 s32Ret = AXCL_ENGINE_Init(&stNpuAttr);
                if (s32Ret == 0) {
