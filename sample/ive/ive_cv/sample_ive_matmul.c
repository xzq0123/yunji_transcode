...
static AX_BOOL SAMPLE_IVE_MemCpy_From_Device(AX_IVE_MAU_BLOB_T *pBlob, size_t Size, AX_IVE_MAU_DATA_TYPE_E enType)
{
    AX_S32 s32TypeSize = SAMPLE_COMM_IVE_Bits_MAU_DataType(enType) / 8;
    if (s32TypeSize == AX_FAILURE)
        return AX_FALSE;

    AX_S32 s32Ret = axclrtMemcpy((AX_VOID *)pBlob->pVirAddr, (AX_VOID *)pBlob->u64PhyAddr, Size * s32TypeSize, AXCL_MEMCPY_DEVICE_TO_HOST);
    if (s32Ret) {
         SAMPLE_IVE_PRT("memcpy device memory %p to host memory %p fail, ret = 0x%x\n",
                        (AX_VOID *)pBlob->u64PhyAddr, (AX_VOID *)pBlob->pVirAddr, s32Ret);
         return AX_FALSE;
    }
    return AX_TRUE;
}
...
static AX_BOOL SAMPLE_IVE_Compare_MauBlopData2(AX_IVE_MAU_BLOB_T *pData1, AX_IVE_MAU_BLOB_T *pData2)
{
    ...
    if (!SAMPLE_IVE_MemCpy_From_Device(pData1, total_elem_num, pData1->enDataType)) {
        return AX_FALSE;
    }
...
static AX_BOOL SAMPLE_IVE_Compare_TopN(AX_IVE_MAU_BLOB_T *pData1, AX_IVE_MAU_BLOB_T *pData2)
{
    ...
    AX_IVE_MAU_BLOB_T *pstBolb = pData1;
    AX_IVE_MAU_DATA_TYPE_E enType = pstBolb->enDataType;
    AX_U8 u8Dims = pstBolb->u8ShapeSize;
    AX_S32 *pShape = &pstBolb->pShape[0];
    AX_U32 u32Size = 1;
    ...
    if (s32Bits < 0)
        return AX_FAILURE;
    AX_S32 s32Ret = axclrtMemcpy((AX_VOID *)pData1->pVirAddr, (AX_VOID *)pData1->u64PhyAddr, u32Size * s32Bits / 8, AXCL_MEMCPY_DEVICE_TO_HOST);
    if (s32Ret) {
         SAMPLE_IVE_PRT("memcpy device memory %p to host memory %p fail, ret = 0x%x\n",
                        (AX_VOID *)pData1->u64PhyAddr, (AX_VOID *)pData1->pVirAddr, s32Ret);
         return AX_FALSE;
    }
...
static AX_S32 Sample_MatMul_Check_Result(TEST_MAT_MUL_T* pstTestMatMul, AX_U32 u32Engine, AX_BOOL bCheck)
{
    if (bCheck) {
        ...
        AX_S32 s32Ret = SAMPLE_COMM_IVE_CreateMauBlobHost(&res_reference, pstTestMatMul->stMatMulOutput.stMulRes.enDataType, ref_shape, 2, AX_FALSE);
        ...
        if (u32Engine != AX_IVE_ENGINE_NPU && pstTestMatMul->stMauMatMulCtrl.bEnableTopNRes) {
            ...
            s32Ret = SAMPLE_COMM_IVE_CreateMauBlobHost(&topn_reference, pstTestMatMul->stMatMulOutput.stTopNRes.enDataType, topn_shape, 2, AX_FALSE);
            if (AX_SUCCESS != s32Ret) {
                SAMPLE_IVE_PRT("Error(%#x),Create mul blob failed!\n", s32Ret);
...
static AX_S32 SAMPLE_IVE_TestMatMulProc(TEST_MAT_MUL_T* pstTestMatMul, AX_U32 u32Engine)
{
    ...
    if (u32Engine != AX_IVE_ENGINE_NPU) {
        ...
        s32Ret = AXCL_IVE_MAU_MatMul(&IveHandle, &pstTestMatMul->stMatMulInput, &pstTestMatMul->stMatMulOutput, &pstTestMatMul->stMauMatMulCtrl, u32Engine, bInstant);
        if (AX_SUCCESS != s32Ret) {
        ...
        s32Ret = AXCL_IVE_NPU_CreateMatMulHandle(&hHandle, &pstTestMatMul->stNpuMatMulCtrl);
        if (AX_SUCCESS != s32Ret) {
            SAMPLE_IVE_PRT("Error(%#x),AX_IVE_NPU_CreateMatMulHandle failed!\n",s32Ret);
            return s32Ret;
        }
        AX_U64 u64StartTime = SAMPLE_COMM_IVE_GetTime_US();
        s32Ret = AXCL_IVE_NPU_MatMul(hHandle, &pstTestMatMul->stMatMulInput, &pstTestMatMul->stMatMulOutput, u32Engine, bInstant);
        if (AX_SUCCESS != s32Ret) {
        ...
        s32Ret = AXCL_IVE_NPU_DestroyMatMulHandle(&hHandle);
        if (AX_SUCCESS != s32Ret) {
