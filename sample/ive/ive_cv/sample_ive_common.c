...
AX_S32 SAMPLE_COMM_IVE_ReadFile(AX_IVE_IMAGE_T* pstImg, FILE* pFp)
{
    ...
    SAMPLE_IVE_PRT("pstImg->enType = %d\n", pstImg->enType);
...
AX_S32 SAMPLE_COMM_IVE_WriteFile(AX_IVE_IMAGE_T* pstImg, FILE* pFp)
{
    ...
    case AX_IVE_IMAGE_TYPE_U8C3_PLANAR:
        for (AX_S32 loop = 0; loop < 3; loop++) {
            fileSZ = pstImg->au32Stride[loop] * height;
            s32Ret = axclrtMemcpy((AX_VOID *)pstImg->au64VirAddr[loop], (AX_VOID *)pstImg->au64PhyAddr[loop], fileSZ, AXCL_MEMCPY_DEVICE_TO_HOST);
            if (s32Ret) {
                 SAMPLE_IVE_PRT("memcpy device memory %p to host memory %p fail, ret = 0x%x\n",
                                (AX_VOID *)pstImg->au64PhyAddr[loop], (AX_VOID *)pstImg->au64VirAddr[loop], s32Ret);
                 return s32Ret;
            }
            pU8 = (AX_U8 *)(AX_UL)pstImg->au64VirAddr[loop];
            for (y = 0; y < height; y++) {
...
AX_S32 SAMPLE_COMM_IVE_CreateMauBlobHost(AX_IVE_MAU_BLOB_T *pstBolb, AX_IVE_MAU_DATA_TYPE_E enType, AX_S32 *pShape, AX_U8 u8Dims, AX_BOOL bRandom)
{
    ...
    AX_S32 s32Ret = axclrtMallocHost((AX_VOID **)&pstBolb->pVirAddr, u32Size * s32Bits / 8);
    if (s32Ret != AX_SUCCESS) {
        SAMPLE_IVE_PRT("Host Cmm Alloc fail,Error(%#x)\n", s32Ret);
        return s32Ret;
    }
...
AX_S32 SAMPLE_COMM_IVE_CreateMauBlob(AX_IVE_MAU_BLOB_T *pstBolb, AX_IVE_MAU_DATA_TYPE_E enType, AX_S32 *pShape, AX_U8 u8Dims, AX_BOOL bRandom)
{
    ...
    if (s32Bits < 0)
        return AX_FAILURE;
    AX_S32 s32Ret = axclrtMallocCached((AX_VOID **)&pstBolb->u64PhyAddr, u32Size * s32Bits / 8, AXCL_MEM_MALLOC_NORMAL_ONLY);
    if (s32Ret != AX_SUCCESS) {
        SAMPLE_IVE_PRT("AXCL_SYS_MemAllocCached fail,Error(%#x)\n", s32Ret);
        return s32Ret;
    }
    s32Ret = axclrtMallocHost((AX_VOID **)&pstBolb->pVirAddr, u32Size * s32Bits / 8);
    if (s32Ret != AX_SUCCESS) {
        SAMPLE_IVE_PRT("Host Cmm Alloc fail,Error(%#x)\n", s32Ret);
        return s32Ret;
    }
    ...
    s32Ret = axclrtMemcpy((AX_VOID *)pstBolb->u64PhyAddr, (AX_VOID *)pstBolb->pVirAddr, u32Size * s32Bits / 8, AXCL_MEMCPY_HOST_TO_DEVICE);
    if (s32Ret) {
         SAMPLE_IVE_PRT("memcpy host memory %p to device memory %p fail, ret = 0x%x\n",
                        (AX_VOID *)pstBolb->pVirAddr, (AX_VOID *)pstBolb->u64PhyAddr, s32Ret);
         return s32Ret;
    }

    axclrtMemFlush((AX_VOID *)pstBolb->u64PhyAddr,  u32Size * s32Bits / 8);
...
AX_VOID SAMPLE_COMM_IVE_FreeMauBlob(AX_IVE_MAU_BLOB_T *pstBolb)
{
    if ((0 != (pstBolb->u64PhyAddr)) && (NULL != (pstBolb->pVirAddr))) {
        AX_S32 s32Ret = AXCL_SYS_MemFree(pstBolb->u64PhyAddr, pstBolb->pVirAddr);
        if (s32Ret != AX_SUCCESS) {
            SAMPLE_IVE_PRT("AXCL_SYS_MemFree fail,Error(%#x)\n", s32Ret);
        }
    }
...
AX_VOID SAMPLE_COMM_IVE_Invalid_MauBlobCache(AX_IVE_MAU_BLOB_T *pstBolb)
{
    ...
    AX_S32 s32Ret = AXCL_SYS_MinvalidateCache(pstBolb->u64PhyAddr, pstBolb->pVirAddr, u32Size * s32Bits / 8);
    if (s32Ret != AX_SUCCESS) {
        SAMPLE_IVE_PRT("AXCL_SYS_MinvalidateCache fail,Error(%#x)\n", s32Ret);
    }
}
