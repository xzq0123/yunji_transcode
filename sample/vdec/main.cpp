...
int main(int argc, char *argv[]) {
    ...
    if (device_id <= 0) {
        ...
        SAMPLE_LOG_I("device id: %d", device_id);
    }

    /* step02: active device */
    SAMPLE_LOG_I("active device %d", device_id);
...
static void sample_get_decoded_image_thread(AX_VDEC_GRP grp, int32_t device_id, axcl::event *eof_event, SAMPLE_VDEC_CHN_INFO chn_info,
    ...
    const struct dma_mem &cma_mem = cma_allocator.get();
    ...
    while (!g_exit) {
        ...
        if (0 == ret) {
            ...
            if (dump) {
                if (axclError Ret =
                        axclrtMemcpy(reinterpret_cast<void *>(cma_mem.phy), reinterpret_cast<void *>(frame.stVFrame.u64PhyAddr[0]), size,
